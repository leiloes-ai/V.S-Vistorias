...
const AccountsPayable: React.FC = () => {
    ...
    const payableAccounts = useMemo(() => {
        ...
        return financials
            .filter(t => t.isPayableOrReceivable && t.type === 'Despesa')
            .map(t => {
    ...
    const filteredPayables = useMemo(() => {
        ...
        return payableAccounts.filter(t => {
    ...
    }, [payableAccounts, searchQuery, statusFilter, thirdParties]);
    
    const summary = useMemo(() => {
        const totalPending = payableAccounts.filter(t => t.status === 'Pendente').reduce((sum, t) => sum + t.amount, 0);
        const totalOverdue = payableAccounts.filter(t => t.status === 'Vencida').reduce((sum, t) => sum + t.amount, 0);
    ...
    const confirmDelete = () => {
        if (transactionToAction) deleteFinancial(transactionToAction.id);
    ...
    return (
        <>
            ...
            <div className="bg-white dark:bg-gray-800 p-4 md:p-6 rounded-xl shadow-lg">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
                    <div className="relative w-full md:w-auto flex-grow">
                        <input type="text" placeholder="Buscar por descrição ou fornecedor..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-primary-500" />
                ...
                {loading ? <p className="text-center py-8">Carregando...</p> : (
                    <div className="overflow-x-auto">
                        ...
                        {!loading && filteredPayables.length === 0 && <p className="text-center py-8">Nenhuma conta a pagar encontrada para os filtros selecionados.</p>}
                    </div>
                )}
            </div>

            <Modal isOpen={isFormModalOpen} onClose={closeFormModal} title={selectedTransaction ? 'Editar Conta a Pagar' : 'Nova Conta a Pagar'}>
                <PayableReceivableForm transaction={selectedTransaction} type="Despesa" onSave={closeFormModal} />
            </Modal>
            ...
            <ConfirmationDialog isOpen={isDeleteConfirmOpen} onClose={() => setIsDeleteConfirmOpen(false)} onConfirm={confirmDelete} title="Confirmar Exclusão" message="Tem certeza que deseja excluir esta conta a pagar?"/>
        </>
    );
};
